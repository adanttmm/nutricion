#!/usr/bin/env bash
# actualizar.sh — flujo completo semanal (báscula → menú → sitio)
# Uso: bash actualizar.sh [--sin-parsear] [--sin-push] [--sin-xiaomi]
#
...
#   bash actualizar_xiaomi.sh [--ambos]       ← sincroniza báscula ATM + IOB
...
for arg in "$@"; do
  case "$arg" in
    ...
    --sin-xiaomi)  SIN_XIAOMI=true ;;
  esac
done

if [[ "$SIN_XIAOMI" == false ]]; then
  bash actualizar_xiaomi.sh
  echo ""
fi
